...
function canonicalize(rows){const by=new Map();for(const r of rows){const old=by.get(r.event_id);if(!old||String(r.started_at)<String(old.started_at)||(String(r.started_at)===String(old.started_at)&&r.arm<old.arm))by.set(r.event_id,r);}return by}
