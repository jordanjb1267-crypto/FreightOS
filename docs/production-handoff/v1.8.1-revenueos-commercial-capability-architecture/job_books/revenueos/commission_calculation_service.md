# Commission Calculation Service — Provisional Audit-Candidate Job Book

**Status:** `AUDIT_CANDIDATE` — not J0, not registered, not enabled, not production-certified.  
**Plane:** `revenueos`  
**Proposed component class:** `deterministic_service`  
**Candidate autonomy ceiling:** `A1_DETERMINISTIC_ROUTING_OR_CALCULATION`  

## Mission

derive commission earnings from immutable events.
...
- payout authority; retroactive ledger rewrite.
...
- `REV-G08` / `C1_EVENT` — `ELIGIBLE_EVENT`
- `REV-G08` / `C2_PLAN` — `PLAN_RESOLVED`
- `REV-G08` / `C4_CALC` — `CALCULATED`
- `REV-G08` / `C7_RECORD` — `PAYOUT_RECORD`
- `REV-G08` / `C8_RECON` — `RECONCILED`
...
- `record_commission_calculation`
- `record_payout_record`
