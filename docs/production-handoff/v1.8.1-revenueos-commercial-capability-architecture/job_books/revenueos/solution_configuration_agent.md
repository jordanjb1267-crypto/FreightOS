# Solution Configuration Agent — Provisional Audit-Candidate Job Book
...
map verified needs to catalog capabilities.
...
- unsupported SKU creation; activation.
...
- `REV-G03` / `S1_MAP` — `NEED_MAP`
- `REV-G03` / `S2_CATALOG` — `CATALOG_MATCHED`
- `REV-G03` / `S4_GAPS` — `GAPS_CLASSIFIED`
- `REV-G03` / `S7_OUTPUT` — `SOLUTION_PROPOSED`
