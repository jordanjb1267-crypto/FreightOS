# Commercial Compliance Guard — Provisional Audit-Candidate Job Book
...
detect promise, authority, privacy, and policy violations.
...
- exception self-approval; legal advice.
...
- `REV-G01` / `R3_ELIGIBILITY` — `OUTREACH_ELIGIBILITY`
- `REV-G02` / `Q5_VALIDATE` — `DISCOVERY_VALIDATED`
- `REV-G03` / `S6_REVIEW` — `SOLUTION_REVIEWED`
- `REV-G04` / `P3_PROMISE` — `PROMISE_CHECKED`
- `REV-G05` / `D3_POLICY` — `REGISTRATION_POLICY`
- `REV-G06` / `I4_VALIDATE` — `HANDOFF_VALIDATED`
- `REV-G07` / `E3_CONSENT` — `OUTREACH_POLICY`
- `REV-G08` / `C5_HOLD` — `PAYOUT_ELIGIBILITY`
- `FMI-G06` / `CR4_PRIVACY` — `DISCLOSURE_CHECK`
- `FMI-G07` / `BA4_POLICY` — `DELIVERY_POLICY`
