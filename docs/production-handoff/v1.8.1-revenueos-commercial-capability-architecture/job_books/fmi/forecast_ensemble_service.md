# Forecast Ensemble Service — Provisional Audit-Candidate Job Book

**Status:** `AUDIT_CANDIDATE` — not J0, not registered, not enabled, not production-certified.  
**Plane:** `fmi`  
**Proposed component class:** `model_deterministic_wrapper`  
**Candidate autonomy ceiling:** `A1_DETERMINISTIC_ROUTING_OR_CALCULATION`  

## Mission

produce versioned calibrated forecasts with uncertainty.
...
- guarantees; business commands; silent model substitution.
...
- `FMI-G05` / `FC1_INPUT` — `FORECAST_INPUT`
- `FMI-G05` / `FC2_MODELS` — `MODEL_RUNS`
- `FMI-G05` / `FC3_ENSEMBLE` — `ENSEMBLED`
- `FMI-G05` / `FC5_LIMITS` — `LIMITATIONS`
