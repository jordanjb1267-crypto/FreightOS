# Market Intelligence Orchestrator — Provisional Audit-Candidate Job Book

**Status:** `AUDIT_CANDIDATE` — not J0, not registered, not enabled, not production-certified.  
**Plane:** `fmi`  
**Proposed component class:** `deterministic_workflow`  
**Candidate autonomy ceiling:** `A1_DETERMINISTIC_ROUTING_OR_CALCULATION`  

## Mission

schedule, partition, and route intelligence WorkUnits.
...
- market conclusion authority by itself; logistics command.
...
- `FMI-G01` / `F2_FETCH` — `INGEST_REQUEST`
- `FMI-G01` / `F3_INGEST` — `RAW_CAPTURE`
- `FMI-G01` / `F5_NORMALIZE` — `NORMALIZED`
- `FMI-G01` / `F6_PUBLISH` — `OBSERVATION_PUBLISHED`
- `FMI-G02` / `N6_PUBLISH` — `NEWS_EVENT_PUBLISHED`
- `FMI-G03` / `RC7_PUBLISH` — `LANE_SIGNAL_PUBLISHED`
- `FMI-G04` / `DV7_PUBLISH` — `REGIME_PUBLISHED`
- `FMI-G05` / `FC6_PUBLISH` — `FORECAST_PUBLISHED`
- `FMI-G06` / `CR6_PUBLISH` — `IMPACT_PUBLISHED`
- `FMI-G08` / `MM5_PUBLISH` — `MAINTENANCE_SIGNAL_PUBLISHED`
- `FMI-G09` / `RP5_PUBLISH` — `POLICY_SIGNAL_PUBLISHED`
- `FMI-G10` / `CX2_IMPACT` — `DEPENDENCY_IMPACT`
- `FMI-G10` / `CX3_MARK` — `SUPERSEDED`
- `FMI-G10` / `CX4_RECOMPUTE` — `RECOMPUTE`
...
- source-registry-permitted market observations;
- provenance/freshness/rights metadata;
...
## FMI-specific evidence contract

Every derived output must preserve source IDs, observed/received time, rights basis, freshness state, transformation/method version, confidence/uncertainty, and correction lineage. News/inbound documents are untrusted content. A forecast must include horizon, model/version, calibration/backtest reference, interval/uncertainty, limitations, and invalidation conditions.

## Source and privacy boundaries

- technical accessibility does not imply permitted ingestion, retention, derivation, training, redistribution, or resale;
- private tenant/network data may be used only for authorized purpose and may not leak into another tenant's market brief;
- thin/conflicting markets must surface uncertainty rather than fabricate precision.
