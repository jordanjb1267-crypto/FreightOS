# Customer Relevance & Impact Agent — Provisional Audit-Candidate Job Book

**Status:** `AUDIT_CANDIDATE` — not J0, not registered, not enabled, not production-certified.  
**Plane:** `fmi`  
**Proposed component class:** `agent`  
**Candidate autonomy ceiling:** `A2_RECOMMEND_OR_BOUNDED_NON_BINDING`  

## Mission

map governed market signals to participant-specific exposure.
...
- Twin mutation; command; cross-tenant disclosure.
...
- `FMI-G06` / `CR1_PROFILE` — `PROFILE_LOAD`
- `FMI-G06` / `CR2_SIGNAL` — `SIGNAL_MATCH`
- `FMI-G06` / `CR3_SCORE` — `RELEVANCE_SCORE`
- `FMI-G06` / `CR5_IMPACT` — `IMPACT_ASSESSMENT`
- `FMI-G08` / `MM4_IMPACT` — `MAINTENANCE_IMPACT`
