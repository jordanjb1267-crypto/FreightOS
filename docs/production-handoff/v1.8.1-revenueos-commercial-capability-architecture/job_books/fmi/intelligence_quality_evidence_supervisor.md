# Intelligence Quality & Evidence Supervisor — Provisional Audit-Candidate Job Book

**Status:** `AUDIT_CANDIDATE` — not J0, not registered, not enabled, not production-certified.  
**Plane:** `fmi`  
**Proposed component class:** `human_supervised_hybrid`  
**Candidate autonomy ceiling:** `A2_HUMAN_SUPERVISED`  

## Mission

detect stale, conflicting, low-quality signals and evaluation drift.
...
- exception self-approval; source-rights override.
...
- `FMI-G01` / `F4_VALIDATE` — `SOURCE_VALIDATED`
- `FMI-G02` / `N3_INJECTION` — `CONTENT_SAFETY`
- `FMI-G02` / `N5_EVENT` — `EVENT_REVIEW`
- `FMI-G03` / `RC3_SAMPLE` — `SAMPLE_QUALITY`
- `FMI-G05` / `FC4_CAL` — `CALIBRATION_CHECK`
- `FMI-G07` / `BA3_QUALITY` — `BRIEF_REVIEW`
- `FMI-G09` / `RP2_VERIFY` — `POLICY_VERIFIED`
- `FMI-G10` / `CX1_TRIGGER` — `CORRECTION_TRIGGER`
- `FMI-G10` / `CX6_CLOSE` — `CORRECTION_RECONCILED`
...
- `none_without_separate_command_contract`
